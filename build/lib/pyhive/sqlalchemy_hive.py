...
class HiveDialect(default.DefaultDialect):
    ...
    def _get_sqla_column_type(self, col_type, col_name):
        try:
            col_type_processed = re.search(r'^\w+', col_type).group(0)
            print(col_type_processed)
            coltype = _type_map[col_type_processed]
            print(coltype)
        except KeyError:
            ...
            if col_type_processed == "RECORD" or col_type_processed == "STRUCT":
                coltype = STRUCT(
                    *(
